...
import { Examen, HojasFirma, ProcesoSelectivo } from "../../../core/api.models";
...
export class FirmasComponent {
  ...
  readonly hojas = signal<HojasFirma | null>(null);
  ...
  selectExamen(examen: Examen): void {
    ...
    this.loading.set(true);
    this.api.getHojasFirma(examen.id).subscribe({
      next: (hojas) => {
        this.loading.set(false);
      ...
      error: (error) => {
        this.loading.set(false);
        this.error.set(errorMessage(error, "No se han podido cargar las hojas de firma."));
      },
    });
  }
}
